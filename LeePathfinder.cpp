...
LeePathfinder::LeePathfinder(std::unique_ptr<Map>& atlas, int size_x, int size_y, int i_start, int j_start, int i_end, int j_end)
{
	this->field = *atlas->Tiles;
	this->size_x = size_x;
	this->size_y = size_y;
...
void LeePathfinder::Backtrace()
{
	...
	int temp = 100;
	path.push_back(Coord(end.first, end.second));
